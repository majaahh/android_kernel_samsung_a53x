 ...
 * &struct KUNIT_SUITE_T and will be run after the module's init function and
 ...
 * in the init function (see &struct KUNIT_SUITE_T).
 *
...
int test_run_tests(struct KUNIT_SUITE_T *module);
...
/**
 * module_test() - used to register a &struct KUNIT_SUITE_T with KUnit.
 * @module: a statically allocated &struct KUNIT_SUITE_T.
 *
 * Registers @module with the test framework. See &struct KUNIT_SUITE_T for more
 * information.
...
#define module_test(module) \
		static struct KUNIT_SUITE_T *__test_module_##module __used       \
...
#define __ASSERT(test, success, message) do {				       \
...
#define ASSERT_FAILURE(test, message) __ASSERT(test, false, message)
