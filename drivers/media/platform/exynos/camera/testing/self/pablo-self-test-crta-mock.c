...
int pst_init_crta_mock(struct is_hardware *hw)
{
	...
	hw->icpu_adt = pablo_malloc(sizeof(struct pablo_icpu_adt), GFP_KERNEL);
	if (!hw->icpu_adt) {
	...
	/* crta_bufmgr mock */
	mock_crta_bufmgr = pablo_malloc(sizeof(struct pablo_crta_bufmgr), GFP_KERNEL);
	if (!mock_crta_bufmgr) {
