...
static void default_frame_work_fn(struct kthread_work *work)
{
	...
	if (!num_repeat)
		num_repeat = frame->stripe_info.region_num;
	else if (frame->stripe_info.region_num)
		num_repeat *= frame->stripe_info.region_num;
	...
	if (num_repeat) {
		...
		while (num_repeat--)
			groupmgr->fn_group_shot(groupmgr, group, frame);
	} else {
		groupmgr->fn_group_shot(groupmgr, group, frame);
	}
}
...
int frame_manager_open(struct is_framemgr *this, u32 buffers)
{
	...
	this->parameters = pablo_zalloc(array_size(sizeof(struct is_param_region), buffers),
				GFP_KERNEL);
	if (!this->parameters) {
