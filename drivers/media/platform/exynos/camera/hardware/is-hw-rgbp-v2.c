							...
							u32 instance, u32 id, u32 set_id, struct is_frame *dma_frame)
{
	...
	ret = rgbp_hw_s_wdma_init(hw_ip, &hw_rgbp->wdma[id], param_set, instance, cmd,
		...
		&comp_sbwc_en, &payload_size, out_yuv);
	if (ret) {
		mserr_hw("failed to initialize RGBP_DMA(%d)", instance, hw_ip, id);
		return -EINVAL;
	}
	...
	if (cmd == DMA_OUTPUT_COMMAND_ENABLE) {
		ret = rgbp_hw_s_wdma_addr(&hw_rgbp->wdma[id], output_dva, 0,
			(hw_ip->num_buffers & 0xffff), 0, comp_sbwc_en, payload_size, out_yuv);
		if (ret) {
			mserr_hw("failed to set RGBP_WDMA(%d) address", instance, hw_ip, id);
			return -EINVAL;
		}
		sbwc_en = comp_sbwc_en ? true : false;
		rgbp_hw_s_sbwc(get_base(hw_ip), set_id, sbwc_en);
	}
	...
	u32 instance, u32 rgbp_strip_start_pos, const struct is_frame *frame, u32 set_id)
{
	...
	rgbp_hw_s_cout_fifo(get_base(hw_ip), set_id, &param_set->otf_output);
	...
	/* set_crop */
	rgbp_hw_s_crop(get_base(hw_ip), in_width, in_height, out_width, out_height);
	...
	struct is_interface_ischain *itfc, int id, const char *name)
{
	...
	hw_ip->pmio_reg_seqs = pablo_zalloc(sizeof(struct pmio_reg_seq) * rgbp_hw_g_reg_cnt(),
			GFP_KERNEL);
