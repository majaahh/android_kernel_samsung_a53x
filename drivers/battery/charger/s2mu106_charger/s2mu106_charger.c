...
static char *s2mu106_otg_supply_list[] = {
	"otg",
};
		...
		int eoc_1st_2nd, int current_limit)
{
	...
	case 1:
		...
		if (!charger->psy_fg)
			...
			power_supply_set_property(charger->psy_fg, POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT, &value);
		}
		break;
	case 2:
		s2mu106_update_reg(charger->i2c, S2MU106_CHG_CTRL10,
...
static void s2mu106_set_bat_ocp(
		struct s2mu106_charger_data *charger, unsigned int ocp_current)
{
	u8 bat_ocp = S2MU106_SET_BAT_OCP_5500mA;
	u8 temp;

	if (ocp_current == 0) {
		/* BAT_OCP 5.5A */
		s2mu106_update_reg(charger->i2c, S2MU106_CHG_CTRL9,
			bat_ocp, SET_BAT_OCP_MASK);
	...
	if (ocp_current > 7000)
		ocp_current = 7000;
	else if (ocp_current < 3500)
		ocp_current = 3500;

	bat_ocp = (ocp_current - 3500) / 500;

	s2mu106_update_reg(charger->i2c, S2MU106_CHG_CTRL9, bat_ocp, SET_BAT_OCP_MASK);
	s2mu106_read_reg(charger->i2c, S2MU106_CHG_CTRL9, &temp);
	pr_info("%s: S2MU106_CHG_CTRL9: 0x%02x\n", __func__, temp);
}
...
static bool s2mu106_chg_init(struct s2mu106_charger_data *charger)
{
	...
	s2mu106_set_bat_ocp(charger, charger->pdata->chg_ocp_current);
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (!mfc_fw_update) {
			...
			if (!psy)
				return -EINVAL;
			ret = power_supply_set_property(psy, (enum power_supply_property)POWER_SUPPLY_EXT_PROP_CHARGE_OTG_CONTROL, &value);
			if (ret < 0)
				pr_err("%s: Fail to execute property\n", __func__);
		...
		struct s2mu106_charger_platform_data *pdata)
{
	...
	if (!np) {
		...
		if (ret) {
			...
			pdata->chg_ocp_current = 5500; /* S2MU106_SET_BAT_OCP_5500mA */
		}

		if (of_property_read_bool(np, "battery,chg_ocp_disable"))
			pdata->chg_ocp_current = 0;
...
static int s2mu106_charger_probe(struct platform_device *pdev)
{
	...
	charger->psy_otg_desc.name           = "s2mu106-otg";
	...
	charger->psy_otg->supplied_to = s2mu106_otg_supply_list;
	charger->psy_otg->num_supplicants = ARRAY_SIZE(s2mu106_otg_supply_list);
